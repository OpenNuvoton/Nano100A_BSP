...
void PowerDown_Check()
{
    ...
    if(CLK->PWRCTL & CLK_PWRCTL_HIRC_EN_Msk)
        printf("\nHIRC Enable");
    else
        printf("\nHIRC Disable");

    if(CLK->PWRCTL & CLK_PWRCTL_LIRC_EN_Msk)
        printf("\nLIRC Enable");
    else
        printf("\nLIRC Disable");
    ...
    if(CLK->PWRCTL & CLK_PWRCTL_WK_DLY_Msk)
        printf("\nWake-up Delay Counter Enable");
    else
        printf("\nWake-up Delay Counter Disable");

    if(CLK->PWRCTL & CLK_PWRCTL_PD_WK_IE_Msk)
