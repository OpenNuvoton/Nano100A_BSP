...
void PDMA_INIT(void)
{
    ...
    PDMA_Open(1 << PDMA_CH);
    u32PDMACnt = 8; //Transmit 2 cycles => 4*2
...
int32_t main (void)
{
    ...
    ADC_Open(ADC, 0, ADC_OPERATION_MODE_CONTINUOUS, ADC_CH_0_MASK|ADC_CH_1_MASK|ADC_CH_2_MASK|ADC_CH_3_MASK);
    ...
    if((g_au32RxPDMADestination[7] & 0xFFF) != ADC_GET_CONVERSION_DATA(ADC, 3))
