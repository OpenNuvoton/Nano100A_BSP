...
{
    uint8_t u8RxData[6], u8TxCnt = 0, u8RxCnt = 0;
    ...
    while(u8RxCnt < 6)
    {
        while(!SPI_GET_RX_FIFO_EMPTY_FLAG(SPI_FLASH_PORT))
            u8RxData[u8RxCnt ++] = SPI_READ_RX(SPI_FLASH_PORT);

        while((!SPI_GET_TX_FIFO_FULL_FLAG(SPI_FLASH_PORT)) && (u8TxCnt < 6))
...
void SpiFlash_ChipErase(void)
{
    ...
    // wait tx finish
    while(!SPI_GET_TX_FIFO_EMPTY_FLAG(SPI_FLASH_PORT));
...
void SpiFlash_NormalPageProgram(uint32_t StartAddress, uint8_t *u8DataBuffer)
{
    uint32_t i = 0;
    uint8_t u8TxCnt = 0;
    uint8_t u8TxData[4] = {0x02, 0x00, 0x00, 0x00};

    SPI_ClearRxFIFO(SPI_FLASH_PORT);

    u8TxData[1] = (StartAddress>>16) & 0xFF;
    u8TxData[2] = (StartAddress>>8) & 0xFF;
    u8TxData[3] = (StartAddress) & 0xFF;
...
void SpiFlash_NormalRead(uint32_t StartAddress, uint8_t *u8DataBuffer)
{
    uint32_t i;
    uint8_t u8TxCnt = 0;
    uint8_t u8TxData[4] = {0x03, 0x00, 0x00, 0x00};
